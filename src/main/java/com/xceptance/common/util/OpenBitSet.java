 ...
 * formats.
 * <p/>
 * <code>OpenBitSet</code> is faster than <code>java.util.BitSet</code> in most operations and
 * *much* faster at calculating cardinality of sets and results of set operations. It can also
...
public class OpenBitSet
{
    ...
    /*
     * // alternate implementation of get() public boolean get1(int index) { int i = index >> 6; //
     * div 64 int bit = index & 0x3f; // mod 64 return ((bits[i]>>>bit) & 0x01) != 0; // this does a
     * long shift and a bittest (on x86) vs // a long shift, and a long AND, (the test for zero is
     * prob a no-op) // testing on a P4 indicates this is slower than (bits[i] & bitmask) != 0; }
     */
    ...
    /*
     * public boolean get2(int index) { int word = index >> 6; // div 64 int bit = index &
     * 0x0000003f; // mod 64 return (bits[word] << bit) < 0; // hmmm, this would work if bit order
     * were reversed // we could right shift and check for parity bit, if it was available to us. }
     */
    ...
    public void flip(long startIndex, long endIndex)
    {
        ...
        /***
         * Grrr, java shifting wraps around so -1L>>>64 == -1 for that reason, make sure not to use
         * endmask if the bits to flip will be zero in the last word (redefine endWord to be the
    ...
    /*
     * public static int pop(long v0, long v1, long v2, long v3) { // derived from pop_array by
     * setting last four elems to 0. // exchanges one pop() call for 10 elementary operations //
     * saving about 7 instructions... is there a better way? long twosA=v0 & v1; long ones=v0^v1;
     * 
