...
import com.xceptance.common.util.OpenBitSet;
...
public class DoubleLowPrecisionValueSet extends AbstractFixedSizeDoubleValueSet
{
    ...
    protected void scale()
    {
        ...
        for (int i = 0; i < numberOfBuckets; i += 2)
        {
        ...
        bitSet.clear(numberOfBuckets >> 1, numberOfBuckets);
    }
    ...
    protected void shift(final int buckets)
    {
        ...
        for (int i = numberOfBuckets - 1; i >= 0; i--)
        {
            if (i >= buckets)
            {
                boolean bitValue = bitSet.fastGet(i - buckets);
                if (bitValue)
                {
                    bitSet.fastSet(i);
                }
