...
@Fork(1)
public class CsvUtilsBenchmarkDecodeField
{
    ...
    private List<List<String>> splitQuotedLines = new ArrayList<>();
    ...
    public void setup()
    {
        ...
        for (String s : lines)
        {
            splitLines.add(com.xceptance.common.csvutils.tuned500.CsvUtils.split(s, ','));
        }
        for (String s : quotedLines)
        {
            splitQuotedLines.add(com.xceptance.common.csvutils.tuned500.CsvUtils.split(s, ','));
        }
    }
    ...
    { 
        ...
        for (List<String> l : splitLines)
        {
            for (String s : l)
            {
                String result = com.xceptance.common.csvutils.tuned500.CsvUtils.decodeField(s);
                sum += result.length();
            }
        }
        return sum;
    ...
    { 
        int sum = 0;
        for (List<String> l : splitQuotedLines)
        {
            for (String s : l)
            {
                String result = com.xceptance.common.csvutils.tuned500.CsvUtils.decodeField(s);
    ...
    { 
        ...
        for (List<String> l : splitLines)
        {
            for (String s : l)
            {
                String result = com.xceptance.common.csvutils.tuned501.CsvUtils.decodeField(s);
                sum += result.length();
            }
        }
        return sum;
    ...
    { 
        ...
        for (List<String> l : splitLines)
        {
            for (String s : l)
            {
                String result = com.xceptance.common.csvutils.tuned501.CsvUtilsDecode.decodeField(s);
                sum += result.length();
            }
        }
        return sum;
