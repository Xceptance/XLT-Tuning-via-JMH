...
public final class CsvUtils
{
     ...
     */
    public static List<String> split(final String s, final char fieldSeparator)
    {
        int length = s.length();
        if (length == 0)
        {
        ...
        final ArrayList<String> target = new ArrayList<>(48);
        ...
        for (int i = 0; i < length; i++)
        {
            ...
            if (c == fieldSeparator)
            {
                ...
                {
                    target.add(new String(src, beginIndex, i - beginIndex));
                    beginIndex = i + 1;
                }
            }
            ...
            // alternative idea to the if statement, hard to benchmark
            // insideQuotes = (c == QUOTE_CHAR) ^ insideQuotes;
        }

        // add the last field
        target.add(new String(src, beginIndex, length - beginIndex));
