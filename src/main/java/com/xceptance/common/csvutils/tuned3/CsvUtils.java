package com.xceptance.common.csvutils.tuned3;
...
public final class CsvUtils
{
    ...
    public static String[] decode(final String s, final char fieldSeparator)
    {
        if (s == null)
        {
            throw new IllegalArgumentException("Parameters s must not be null");
        }
    ...
    private static String[] split(final String s, final char fieldSeparator)
    {
        ...
        for (int i = 0; i < length; i++)
        {
            ...
            if (c == fieldSeparator)
            {
                ...
                {
                    target.add(decodeField(s.substring(beginIndex, i)));
                    beginIndex = i + 1;
                }
            }
        ...
        // add the last field
        target.add(decodeField(s.substring(beginIndex)));
