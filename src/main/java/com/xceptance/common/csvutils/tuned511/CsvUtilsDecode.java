package com.xceptance.common.csvutils.tuned511;
...
public final class CsvUtilsDecode
{
     ...
     */
    public static List<CharBuffer> parse(final String s)
    {
     ...
     */
    public static List<CharBuffer> parse(final char[] _src, final char fieldSeparator)
    {
        final SimpleArrayList<CharBuffer> result = new SimpleArrayList<>(32);
        final CharBuffer src = CharBuffer.wrap(_src);
        ...
        for (int i = 0; i < src.length(); i++)
        {
            ...
            if (c == fieldSeparator)
            {
                if ((state & IN_QUOTES) == 0)
                {
                    ...
                    result.add(pos == 0 ? src.subSequence(0, 0) : src.subSequence(start, pos));
        ...
        if (pos > 0)
        {
            result.add(src.subSequence(start, pos));
        }
        ...
        {
            result.add(src.subSequence(0, 0));
        }
