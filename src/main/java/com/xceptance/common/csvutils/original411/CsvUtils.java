...
public final class CsvUtils
{
    ...
    public static String decodeField(final String s)
    {
        ...
        if (sourceChars[0] != QUOTE_CHAR || sourceChars[length - 1] != QUOTE_CHAR)
        {
            return s;
        }

        final char[] targetChars = new char[length - 2];
        ...
        for (i = 1, j = 0; i < length - 1; i++, j++)
        {
            ...
            if (c == QUOTE_CHAR)
            {
                ...
                i++;
                if (i >= length - 1 || sourceChars[i] != QUOTE_CHAR)
                {
        ...
        return new String(targetChars, 0, j);
    }
    ...
    private static String[] split(final String s, final char fieldSeparator)
    {
        ...
        final String[] fields = new String[s.length() + 1];
        ...
        for (int i = 0; i < chars.length; i++)
        {
            ...
            if (c == fieldSeparator)
            {
                if (!insideQuotes)
                {
                    fields[numberOfFieldsFound++] = s.substring(beginIndex, i);
                    beginIndex = i + 1;
                }
            }
        ...
        // add the last field
        fields[numberOfFieldsFound++] = s.substring(beginIndex);

        // get a sub array with only the fields found
        final String[] f = new String[numberOfFieldsFound];
        System.arraycopy(fields, 0, f, 0, numberOfFieldsFound);
